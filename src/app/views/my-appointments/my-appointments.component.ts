...
export class MyAppointmentsComponent implements OnInit {
  ...
  getAppointments(){
    this.appointmentService.getAppointments(this.user.id, this.page-1, this.size).subscribe(
      data =>{
        console.log(data);
  ...
  checkActive(appointment:any):number{
    ...
    if(!validDate && appointment.status === 1)
      return 1 
    if(!validDate && appointment.status === 2)
      return 3
    return 2
  }
  cancelAppointment(appointment:any){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.appointmentService.cancelAppointment(cancelation).subscribe(
          data =>{
            Swal.fire(
              'Cancelled!',
